...
class Gleason(BaseDataset):
    def __init__(self, 
        ...
        super(Gleason, self).__init__(ignore_label, base_size,
        ...
        self.class_weights = torch.FloatTensor([2.0, 3.84, 6.25, 14.3, 166.7]).cuda()
        self.multi_crops = [(625, 625), (1250, 1250), (2500, 2500), (5000, 5000)]
        self.n_x = 1
        self.n_y = 1
    ...
    def __getitem__(self, index):
        ...
        name = item["name"]
        image = cv2.imread(os.path.join(self.root,'gleason',item["img"]),
                           cv2.IMREAD_COLOR)
        size = image.shape

        label = cv2.imread(os.path.join(self.root,'gleason',item["label"]),
                           cv2.IMREAD_COLOR)
