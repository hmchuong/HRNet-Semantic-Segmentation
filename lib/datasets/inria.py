...
class Inria(BaseDataset):
    def __init__(self, 
        ...
        super(Inria, self).__init__(ignore_label, base_size,
    ...
    def __getitem__(self, index):
        ...
        name = item["name"]
        image = cv2.imread(os.path.join(self.root,'inria',item["img"]),
                           cv2.IMREAD_COLOR)
        size = image.shape

        label = cv2.imread(os.path.join(self.root,'inria',item["label"]),
                           cv2.IMREAD_COLOR)
