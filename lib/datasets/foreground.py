...
class Foreground(BaseDataset):
    def __init__(self, 
        ...
        self.original_size = (5000,5000)
        super(Foreground, self).__init__(ignore_label, base_size,
    ...
    def __getitem__(self, index):
        ...
        name = item["name"]
        image_path = os.path.join(self.root,'foreground',item["img"])
        gt_path = os.path.join(self.root,'foreground',item["label"])
        # if not os.path.isfile(image_path) or not os.path.isfile(gt_path):
        #     print(image_path)
        #     print(gt_path)
        image = cv2.imread(os.path.join(self.root,'foreground',item["img"]),
                           cv2.IMREAD_COLOR)
        size = image.shape

        label = cv2.imread(os.path.join(self.root,'foreground',item["label"]),
                           cv2.IMREAD_COLOR)
        
        # if self.crop_size != self.original_size:
        #     x = (patch_idx // self.n_x) * int(self.crop_size[1])
        #     y = (patch_idx % self.n_x) * int(self.crop_size[0])
